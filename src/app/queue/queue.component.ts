  ...
  sabtDarkhast() {
    ...
    this.apiService.checkUserHasNoActiveCrossRequest(noeDarkhast).subscribe(
          ...
          if (noeDarkhast === 0) {
            this.apiService.sabtDarkhastKharid(darkhast).subscribe(
              () => {
                this.getSafeKharid();
                this.toastr.success(
                  'درخواست خرید شما با موفقیت به صف خرید افزوده شد'
                );
              },
              error => {
                console.log(error);
            ...
            this.apiService.getUserPortfoDarayi().subscribe(
              data => {
                ...
                this.apiService.getTedadKolSahamForushUser().subscribe(
                  ...
                  error => {
                    console.log(error);
                    this.toastr.error(
                      'خطا در محاسبه کل سهام صف فروش کاربر.با پشتیبان سیستم تماس بگیرید.'
                    );
                  }
                );
              },
              error => {
                console.log(error);
                this.toastr.error(
                  'خطا در بازیابی دارایی سهام.با پشتیبان سیستم تماس بگیرید.'
                );
              }
            );
          }
      ...
      error => {
        console.log(error);
        this.toastr.error(
          'خطا در افزودن درخواست به صف .با پشتیبان سامانه تماس بگیرید'
        );
      }
    );
  }
