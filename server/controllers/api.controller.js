...
module.exports = function (app) {
  ...
  app.route('/api/noeFile/:id')
    .put(checkIsAuthenticated, update_noeFile)
    .delete(checkIsAuthenticated, delete_noeFile);
  ...
  // Aggregation methods
  app.route('/api/aggregate/darkhast/saff')
    .get(get_darkhast_saff)
};
...
get_darkhast_saff = async (req, res) => {
  try {
    const darkhast = await context.Darkhast.aggregate([
      {
        $match: {
          vazeiat: { $in: ['در انتظار', 'در حال انجام'] }
        }
      },
      {
        $sort: {
...
delete_noeFile = function (req, res) {
  context.UserFile.findOne({
    "noeFile": req.params.id
  }, (err, noeFile) => {
    if (err) res.send(err);
    if (noeFile) {
